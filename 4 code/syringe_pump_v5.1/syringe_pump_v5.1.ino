...
// Design inspired by the OpenSyringePump developed by Naroom, https://github.com/manimino/OpenSyringePump.
...
Adafruit_StepperMotor *myMotorH = AFMS.getStepper(200, 1);      // Stepper motor for the rotating frames, facing sideways
...
float FlowRate = 5;                       // How fast the dope is pushed out of the syringe in µL/min
...
const float SyringeRadius = 0.205;        // The inner radius of the syringe in mm
...
const int Rebound = 2000;                 // Average time spent holding down the buttons

float VolumePerRevolution;                // The volume pushed per 200 steps
float VolumePerStep;                      // The volume pushed per 1 step
...
float VTimeForAllSteps;                   // The time it takes to do all microsteps not including the delay inbetween for the syringe (vertical) motor
